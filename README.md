...
- at least 2 vCPUs and 8 GB RAM; 4 vCPUs and 8 GB RAM are recommended
...
The launcher assigns Android up to 4 host CPUs and 4 GB RAM. It refuses to run on hosts below 7 GB because Android 16 raises smaller guest-memory settings internally and becomes unstable once SwiftShader and the remote desktop are included. Optional `AVD_CPU_CORES` and `AVD_RAM_MB` values in `.env` can lower allocations on larger hosts, but cannot exceed the detected host resources.
